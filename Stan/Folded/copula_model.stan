functions {
  ...
  vector fold_data(vector x, int n) {
    vector[4 * n * n] folded;
    for (i in 1:n) {
      for (j in 1:n) {
        int idx = (i-1)*n + j;
        folded[(i-1)*2*n + j] = x[idx];
        folded[(i-1)*2*n + (2*n - j + 1)] = x[idx];
        folded[(2*n - i)*2*n + j] = x[idx];
        folded[(2*n - i)*2*n + (2*n - j + 1)] = x[idx];
      }
    }
    return folded;
  }
...
model {
  matrix[2 * dim, 2 * dim] c = create_base_matrix(2 * dim, rho);
  complex_matrix[2 * dim, 2 * dim] eigs = compute_and_rescale_eigenvalues(c, nu);
  matrix[D, n_obs] u;
  ...
  for (i in 1:n_obs) {
    vector[4 * D] Z_folded = fold_data(Z[ , i], dim);
    vector[4 * D] Qx = matvec_prod(eigs, Z_folded);
    target += - (dot_product(Z_folded, Qx) - log_det - sum(square(Z_folded))) / 8.0;
  }
